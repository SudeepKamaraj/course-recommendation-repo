...
const AuthModal: React.FC<AuthModalProps> = ({ onClose }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (success) {
        // Persist user preferences locally for recommendations
        localStorage.setItem('user_interests', formData.interests || '');
        localStorage.setItem('user_career_objective', formData.careerObjective || '');
        onClose();
      }
    } catch (err) {
          ...
          <div className="mt-6 text-center">
            <button
              onClick={() => {
                setIsLogin(!isLogin);
                setError('');
                setFormData({ name: '', email: '', password: '', interests: '', careerObjective: '' });
                setSelectedSkills([]);
              }}
