...
const Recommendations: React.FC<RecommendationsProps> = ({ onCourseSelect }) => {
  ...
  const recommendedCourses = getRecommendedCourses(user.skills || [], user.learningHistory || [])
    .map((course) => {
      // Simple boost if course skills or title/description match interests or objective
      const haystack = `${course.title} ${course.description} ${course.skills.join(' ')}`.toLowerCase();
      const interestTokens = interests.toLowerCase().split(/[,\s]+/).filter(Boolean);
      const objectiveTokens = careerObjective.toLowerCase().split(/[,\s]+/).filter(Boolean);
      const tokens = new Set([...interestTokens, ...objectiveTokens]);
      let boost = 0;
      tokens.forEach((t) => {
        if (haystack.includes(t)) boost += 0.5;
      });
      return { course, boost };
    })
  ...
  const trendingCourses = courses.filter(course => course.students > 10000).slice(0, 4);
