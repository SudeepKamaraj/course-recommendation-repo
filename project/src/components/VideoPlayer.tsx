...
import { ArrowLeft, Play, Pause, Volume2, Maximize2, Check, Settings, Download, SkipForward, SkipBack, RotateCcw, Sparkles } from 'lucide-react';
...
const VideoPlayer: React.FC<VideoPlayerProps> = ({
  ...
  const { user } = useAuth();
  const { updateLessonProgress } = useCourses();
  ...
  // Get video metadata from service
  const videoMetadata = VideoService.getVideoMetadata(lesson.id);
  const videoUrl = VideoService.getVideoUrl(lesson.id);
  const videoDuration = VideoService.getVideoDuration(lesson.id);
  ...
  const skipForward = () => {
    const video = videoRef.current;
    if (!video) return;
    // Disable skipping in fullscreen mode
    if (isFullscreen) {
      return;
    }
              ...
              <div className="flex items-center justify-between text-white">
                <div className="flex items-center space-x-4">
                  <button
                    ...
                    title={isFullscreen ? 'Skipping disabled in fullscreen mode' : 'Skip backward 10s'}
                  >
                  ...
                  <button
                    ...
                    title={isFullscreen ? 'Skipping disabled in fullscreen mode' : 'Skip forward 10s'}
                  >
